...
#
# Runs as the openclaw user via cron (every 15 min). Uses scoped sudoers
# for systemctl commands (/etc/sudoers.d/openclaw-deploy).

DEPLOY_DIR="/opt/sherpa"
REPO_DIR="/home/openclaw/sherpa"
BLUE_PORT=3000
GREEN_PORT=3001
HEALTH_TIMEOUT=30
...
if [ "${1:-}" = "--rollback" ]; then
  ...
  sudo systemctl start "sherpa-studio@$STANDBY"
  ...
  for i in $(seq 1 $HEALTH_TIMEOUT); do
    ...
    [ "$i" -eq "$HEALTH_TIMEOUT" ] && { err "Rollback health check failed"; sudo systemctl stop "sherpa-studio@$STANDBY" 2>/dev/null || true; exit 1; }
    sleep 1
  done
  ...
  sleep 2
  sudo systemctl stop "sherpa-studio@$ACTIVE" 2>/dev/null || true
...
for i in $(seq 1 $HEALTH_TIMEOUT); do
  ...
  if [ "$i" -eq "$HEALTH_TIMEOUT" ]; then
    ...
    sudo systemctl stop "sherpa-studio@$STANDBY" 2>/dev/null || true
    exit 1
  fi
  sleep 1
done
...
sudo systemctl restart sherpa-mcp

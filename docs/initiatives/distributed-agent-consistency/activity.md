...
- 2026-03-12: Full iteration 1 complete with web-sourced research. 5 parallel vectors: actor model, optimistic concurrency, consensus protocols, multi-agent framework analysis, event sourcing. 7 research reports (4000+ lines, 200+ URLs). Core synthesis: Sherpa is a blackboard system with stigmergic coordination — validated since the 1970s. Consensus overkill; POSIX atomic primitives are the right foundation. Proposal written: 5-phase consistency layer (atomic writes → ETags → version vectors → JSONL events → coordination locks). 2 branch seeds: agent-failure-supervision, mock-agent-test-harness.

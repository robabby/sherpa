...
> Branched from `origin/main` (not from Phase 4's `feat/audit-fixes-4`) to avoid stacking on the unmerged #38 — the same stacked-base hazard that auto-closed #35. Phase 5's files don't depend on Phase 4's. Note for whoever merges second: both #38 and this PR append a new entry to the end of this activity.md, so the second merge will hit a trivial end-of-file conflict — keep both entries in chronological order.
